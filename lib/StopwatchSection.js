...
class StopwatchSection extends CompositeStopwatch {
     ...
     */
    getEventCount() {
        return this.events.length + super.getEventCount();
    }
    ...
    flatten(clearEvents = false) {
        ...
        if (clearEvents) {
            this.events.forEach(event => event.periods = []);
        }
        let nestedFlat = super.flatten(clearEvents);
        return [json].concat(...nestedFlat);
    }
