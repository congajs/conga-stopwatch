...
class CompositeStopwatch {
    ...
    /**
     * See if this section has any events
     * @returns {boolean}
     */
    hasEvents() {
    ...
    /**
     * Get the total event count (including all nested sections)
     * @returns {Number}
     */
    getEventCount() {
        let num = 0;
        for (const section of this.sections) {
            num += section.getEventCount();
        }
        return num;
    }
     ...
     * @param {Boolean} [clearEvents] True to also clear (remove) the event periods from each event as we recursively flatten
     * @returns {Array<Object>} Array of section objects, containing all event JSON payloads
     */
    flatten(clearEvents = false) {
        let flat = [];
        for (const section of this.sections) {
            const flatSection = section.flatten(clearEvents);
            flat = flat.concat(...flatSection);
        }
        return flat;
    }
